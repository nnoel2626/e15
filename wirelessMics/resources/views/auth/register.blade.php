                ...
                <h2 dusk="register-heading" class="active"> {{ __('Register') }} </h2>
                 <form method="POST" action="{{ route('register') }}">
                     @csrf
                        ...
                        <input dusk="name-input" type="name" class="text" name="name" value="{{ old('name') }}" required autocomplete="name">
                        @include('includes.error-field', ['fieldName' => 'name'])

                        <label for="email"> <span>{{ __('E-Mail Address') }}</span></label>
                        <input dusk="email-input" type="email" class="text" name="email" value="{{ old('email') }}" required autocomplete="email">
                        ...
                        <input dusk="password-confirm-input" id='password-confirm' type="password" class="text" name="password_confirmation" required>
